...
def fetch_books(page=0):
    r = requests.get(
        f"https://www.gutenberg.org/ebooks/search/?sort_order=random")
...
def pipeline(num_books, folder):
    ...
    if os.path.exists(f"log_books.txt"):
        f = open(f"log_books.txt")
        for line in f.readlines():
            line_str = str(line).strip().replace("\x00", "")
            match = re.search('/ebooks/(\d+)',
                              line_str, re.IGNORECASE)
            if match:
                fetched_books.add(match.group(1))
    else:
        for file in os.listdir(f"output/{folder}"):
            os.remove(f"output/{folder}/{file}")

    print(f"Already fetched {len(fetched_books)} books")
    scrapped_books = len(fetched_books)
