...
def fetch_books(page=0):
    r = requests.get(f"https://www.gutenberg.org/ebooks/search/?sort_order=downloads&start_index={page*25+1}")
    soup = BeautifulSoup(r.text, 'html.parser')

    links = [a['href'] for a in soup.find_all('a', href=True)]
    links = [link for link in links if link.startswith('/ebooks/') and len(link) > 8 and link[8].isdigit()]
...
def fetch_book_info_table(book_id):
    ...
    for row in rows:
        key = row[0].text
        data[key] = data.get(key, []) + [aux_parse_column(column) for column in row[1:]]
...
def fetch_book_txt(book_id):
    ...
    r = requests.get(f"https://www.gutenberg.org/cache/epub/{book_id}/pg{book_id}.txt") # TODO this link looks better updated (not cached, so less errors) but have to be careful with encoding mode https://www.gutenberg.org/cache/epub/{book_id}/pg{book_id}.txt
    ...
    if len(text) != 2:
        print(f"WARNING: Book {book_id} has no start of the project gutenberg ebook")
    
    text = text[1]
    text = re.split(r'\*\*\*\s?END[^*]*\*\*\*\s', text, maxsplit=1)
    if len(text) != 2:
        print(f"WARNING: Book {book_id} has no end of the project gutenberg ebook")
...
def parse_book_info_table(data):
    ...
    if 'Author' not in data:
        if 'Translator' in data: # TODO should we do this?
            print(f"WARNING: No author found for book {book_id}. Implicitely replacing with 'Translator' {data['Translator']}...")
            data['Author'] = data['Translator']

    EXPECT_EXACTLY_ONE = ['Title', 'Language', 'Release Date', 'Copyright Status', 'EBook-No.']
    ...
    if book_id != data['EBook-No.'][0]['text']:
        print(f"WARNING: Book ID mismatch for book {book_id}: {data['EBook-No.'][0]['text']}")
    
    if data['Copyright Status'][0]['text'] != 'Public domain in the USA.':
        print(f"WARNING: Unexpected copyright status for book {book_id}: {data['Copyright Status'][0]['text']}")
    ...
    def aux_parse_author(author):
        ...
        if len(author_href_parts) == 0:
            print(f"WARNING: Invalid author ID for book {book_id}: {author_href_parts}")
        
        if len(author_text_parts) == 2:
            print(f"WARNING: Author is missing a first name {book_id}: {author_text_parts}")
            author_text_parts = [author_text_parts[0], None, author_text_parts[1]]
            author_text_parts[0] = author_text_parts[0].strip()
        elif len(author_text_parts) != 3:
            print(f"WARNING: Invalid author name structure for book {book_id}: {author_text_parts}")
        else:
            author_text_parts[0] = author_text_parts[0].strip()
        ...
        if len(author_living_years) != 2:
            print(f"WARNING: Invalid author living years structure for book {book_id}: {author_living_years}")
