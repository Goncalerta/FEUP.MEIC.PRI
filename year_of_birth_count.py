    ...
    for entry in os.listdir(SOURCE_DIR):
        with open(SOURCE_DIR + entry, 'r') as file:
            ...
            if len(authors) == 0:
                ...
                continue
            if len(authors) != 1:
                warn(f'Multiple authors found for book entry {entry}! Will use first one.')
            author = authors[0]
            birth_info = author['year_of_death']
            if not birth_info:
                warn(f'No birth year was found on book entry {entry}. Ignoring..')
                continue
            ...
            for uncertainty_tag in UNCERTAINTY_TAGS:
                birth_info, matched = simplify_birth_info(birth_info, uncertainty_tag)
            ...
            if not birth_info.isdigit():
                warn(f"Non-standard birth info found: {birth_info}\nIgnoring..")
                continue
